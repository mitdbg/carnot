...
def render_cache_path(renders_dir: str | Path, month: str, page: int) -> Path:
    """The page's image-cache path: `<renders_dir>/<month>_<page>.png` — flat, underscore-
    joined (the layout the shared `page_renders/` render farm uses), so this cache
    interoperates with pre-rendered farms rather than a `<month>/` subdir."""
