...
One entry point (`main` / `python -m officeqa.page_index.pipeline`) runs every stage in
...
per-bulletin (`BulletinStage`); two are whole-corpus reductions:
  ...
  2. `vision_rescan` (reduction)               — re-scan each page the text scan flagged
     ...
     standalone `officeqa.page_index.prep.render_corpus` tool (cold pages are rendered on demand).
...
log = logging.getLogger("officeqa.page_index.pipeline")
