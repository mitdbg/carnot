...
cache that `PageStore` (query-time vision) and the build's `vision_rescan` stage both read.
  ...
  python3 -m officeqa.page_index.prep.render_corpus                       # all bulletins, one proc/core
  python3 -m officeqa.page_index.prep.render_corpus --workers 8
  python3 -m officeqa.page_index.prep.render_corpus --only 1990-09,1992-03  # smoke test
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i in range(workers):
        cmd = [
            sys.executable,
            "-m",
            "officeqa.page_index.prep.render_worker",
