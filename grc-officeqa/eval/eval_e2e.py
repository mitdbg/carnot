...
# grc-officeqa app root (this file lives at <root>/eval/eval_e2e.py). Put it on
# sys.path so the `officeqa` package and `eval.*` import without an install step.
...
# Layered setdefault: the app's own .env wins, the skunk library checkout's .env
# (where the keys historically live) fills anything unset.
_load_env(REPO_ROOT / ".env")
_load_env(REPO_ROOT.parent / "skunk" / ".env")
