...
def populate_template(template: str, variables: dict[str, Any]) -> str:
    ...
    try:
        ...
        raise Exception(f"Error during jinja template rendering: {type(e).__name__}: {e}") from e
...
type StreamEvent = ChatMessageStreamDelta | ChatMessageToolCall | ActionOutput | ToolCall | ToolOutput | PlanningStep | ActionStep | FinalAnswerStep


class BaseAgent(ABC):
    """
    NOTE: based on the MultiStepAgent provided by smolagents library.
    ...
    def run(
        ...
        Example:
        ```py
        from carnot.agents import CodeAgent
...
{str(additional_args)}."""
    ...
    def _validate_final_answer(self, final_answer: Any):
        for check_function in self.final_answer_checks:
            try:
                ...
                raise AgentError(f"Check {check_function.__name__} failed with error: {e}", self.logger) from e
    ...
    @classmethod
    def from_dict(cls, agent_dict: dict[str, Any], **kwargs) -> "BaseAgent":
        ...
        Returns:
            `BaseAgent`: Instance of the agent class.
        """
        # Load model
        model_info = agent_dict["model"]
        model_class = getattr(importlib.import_module("carnot.agents.models"), model_info["class"])
        ...
        for managed_agent_dict in agent_dict["managed_agents"]:
            agent_class = getattr(importlib.import_module("carnot.agents.agents"), managed_agent_dict["class"])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            resources.files("carnot.agents.prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
...
class CodeAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._use_structured_outputs_internally:
            prompt_templates = prompt_templates or yaml.safe_load(
                resources.files("carnot.agents.prompts").joinpath("structured_code_agent.yaml").read_text()
            )
        else:
            prompt_templates = prompt_templates or yaml.safe_load(
                resources.files("carnot.agents.prompts").joinpath("code_agent.yaml").read_text()
            )
    ...
    def _step_stream(
        ...
        try:
            ...
            if not self._use_structured_outputs_internally and output_text and not output_text.strip().endswith(self.code_block_tags[1]):
        ...
        try:
            if self._use_structured_outputs_internally:
                ...
                code_action = extract_text_from_tags(code_action, self.code_block_tags) or code_action
            else:
