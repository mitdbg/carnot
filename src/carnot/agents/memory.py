...
from carnot.agents.models import ChatMessage, MessageRole
from carnot.agents.monitoring import AgentLogger, LogLevel, Timing, TokenUsage
from carnot.agents.tools import Tool
from carnot.agents.utils import AgentError, make_json_serializable
...
@dataclass
class CodeOperatorStep(MemoryStep):
    task: str
    tools: dict[str, Tool]
...
@dataclass
class SemFilterOperatorStep(MemoryStep):
    task: str
    item: dict
...
@dataclass
class SemMapOperatorStep(MemoryStep):
    task: str
...
@dataclass
class SemFlatMapOperatorStep(MemoryStep):
    task: str
    output_fields: list[dict]
    item: dict
...
class SemJoinOperatorStep(MemoryStep):
    ...
    def to_messages(self, summary_mode = False) -> list[ChatMessage]:
        left_input_str = json.dumps(self.left_item, indent=2)
        right_input_str = json.dumps(self.right_item, indent=2)
        content = f"Join Condition: \"{self.task}\"\n\nLeft Input:\n{left_input_str}\n\nRight Input:\n{right_input_str}"
        return [ChatMessage(role=MessageRole.USER, content=[{"type": "text", "text": content}])]
