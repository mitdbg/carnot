...
def parse_json_blob(json_blob: str) -> tuple[Any, str]:
    "Extracts the JSON blob from the input and returns the JSON data and the rest of the input."
    try:
        first_obj_index = json_blob.find("{")
        first_arr_index = json_blob.find("[")

        if first_obj_index == -1 and first_arr_index == -1:
            raise ValueError("The model output does not contain any JSON blob.")

        if first_obj_index == -1 or (0 <= first_arr_index < first_obj_index):
            start_index = first_arr_index
            end_index = [a.start() for a in list(re.finditer(r"\]", json_blob))][-1]
        else:
            start_index = first_obj_index
            end_index = [a.start() for a in list(re.finditer(r"\}", json_blob))][-1]

        json_str = json_blob[start_index : end_index + 1]
        json_data = json.loads(json_str, strict=False)
        return json_data, json_blob[:start_index]
    except IndexError as e:
        raise ValueError("The model output does not contain any JSON blob.") from e
...
def parse_json_output(text: str, output_tags: tuple[str, str]) -> dict[str, Any]:
    """
    Extract the JSON output from the LLM's output.

    Args:
        text (`str`): LLM's output text to parse.
        output_tags (`tuple[str, str]`): Opening and closing tags for the output block.
    
    Returns:
        `dict[str, Any]`: Extracted JSON output
    """
    output_text = extract_text_from_tags(text, output_tags)
    if output_text:
        try:
            json_data, _ = parse_json_blob(output_text)
            return json_data
        except json.JSONDecodeError as e:
            raise ValueError(f"Parsed JSON output is not valid JSON. Parsed output: {output_text}") from e

    raise ValueError(
        dedent(
            f"""
            Your output snippet is invalid, because the regex pattern {output_tags[0]}(.*?){output_tags[1]} was not found in it.
            Here is your output snippet:
            {text}
            Make sure to include the output with the correct pattern, for instance:
            {output_tags[0]}
            {{
                "key": "value"
            }}
...
def parse_boolean_output(text: str, output_tags: tuple[str, str]) -> bool:
    """
    Extract the booelan decision from the LLM's output.
    ...
    Returns:
        `bool`: Extracted boolean decision
    """
    output_text = extract_text_from_tags(text, output_tags)
    if output_text:
        true_in_output = "true" in output_text.lower()
        false_in_output = "false" in output_text.lower()
        if not true_in_output and not false_in_output:
            raise ValueError(f"Parsed boolean output does not contain TRUE or FALSE. Parsed output: {output_text}")
        elif true_in_output and false_in_output:
            raise ValueError(f"Parsed boolean output contains both TRUE and FALSE. Only one of these may be present in the output. Parsed output: {output_text}")
        else:
            return true_in_output
    ...
    raise ValueError(
        dedent(
            ...
            Make sure to include the output with the correct pattern, for instance:
            {output_tags[0]}
            # Your output here (either TRUE or FALSE)
            {output_tags[1]}
            """
        ).strip()
    )

def parse_plan(text: str, plan_tags: tuple[str, str]) -> str:
    """
    Extract the plan from the LLM's output.

    If a valid plan block is passed, it returns it directly.

    Args:
        text (`str`): LLM's output text to parse.
        plan_tags (`tuple[str, str]`): Opening and closing tags for the plan block.
    
    Returns:
        `str`: Extracted plan block.
    """
    output_text = extract_text_from_tags(text, plan_tags)
    if output_text:
        return output_text
    raise ValueError(
        dedent(
            f"""
            Your plan snippet is invalid, because the regex pattern {plan_tags[0]}(.*?){plan_tags[1]} was not found in it.
            Here is your plan snippet:
            {text}
            Make sure to include the plan with the correct pattern, for instance:
            {plan_tags[0]}
            # Your plan here
            {plan_tags[1]}
            """
...
def parse_code_blobs(text: str, code_block_tags: tuple[str, str]) -> str:
    ...
    if not output_code_blobs:  # Fallback to markdown pattern
