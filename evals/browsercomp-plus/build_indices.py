...
def load_corpus(corpus_path: str) -> list[dict]:
    """Load BrowserComp+ corpus from JSONL."""
    items = []
...
def build_flat_index(
    ...
    # Add uri and path so summary-based indices can cache per-document state.
    items_with_paths = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Build indices for BrowserComp+ evaluation"
    )
    parser.add_argument(
        "--corpus-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Dataset ID for index naming (must match eval_indices.py). "
