...
def download_from_google_drive():
    ...
    if audio_path.exists() and image_path.exists():
        print("Data already available at:")
...
def _ensure_q6_pattern(audio_df: pd.DataFrame, image_df: pd.DataFrame) -> tuple:
    ...
    for city in target_cities:
        ...
        if len(city_monkey_images) == 0:  # noqa: SIM102
...
def _ensure_cooccurrence_patterns(audio_df: pd.DataFrame, image_df: pd.DataFrame) -> tuple:
    ...
    for city in cooccurrence_cities:
        ...
        if len(zebra_rows[zebra_rows['City'] == city]) == 0:  # noqa: SIM102
        ...
        if len(impala_rows[impala_rows['City'] == city]) == 0:  # noqa: SIM102
...
if __name__ == '__main__':
    ...
    base_folder = Path(__file__).resolve().parents[3] / "files" / "animals" / "data"
