...
class EcommScenario:
        ...
        for file in query_files:
            with open(file, "rb") as f:
                try:
                    ...
                    raise ValueError(f"Error decoding TOML file {file}: {e}") from e
    ...
    def setup_scenario(self, systems: list[str]) -> None:
        ...
        for system in systems:
            if system in ["bigquery", "snowflake", "thalamusdb"]:
                pass
            elif system == "lotus":
    ...
    def get_accuracy_measure_for_query(self, query_id: int) -> str:
        ...
        try:
            ...
            raise KeyError(f"Missing key {e} for query ID {query_id}.") from e
