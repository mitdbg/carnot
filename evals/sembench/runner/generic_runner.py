...
class GenericRunner(ABC):
    ...
    def run_all_queries(self, queries: list[int] | None = None) -> dict[int, GenericQueryMetric]:
        """
    ...
    def load_data(self, filename: str, **kwargs) -> pd.DataFrame:
        ...
        Args:
            filename: Name of the data file (CSV or Parquet)
        ...
        if data_file.suffix in [".parquet", ".pq"]:
            return pd.read_parquet(data_file, **kwargs)
