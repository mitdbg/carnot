...
class GenericEvaluator(abc.ABC):
    ...
    def __init__(self, use_case: str, scale_factor: int) -> None:
        self._root = Path(__file__).resolve().parents[1] / "files" / use_case
    ...
    def evaluate_system(
        self, system_name: str, queries: list[int] | None = None
    ) -> None:
    ...
    def compute_f1_score_classify(
        ...
        if ground_truth.shape[0] != query_result.shape[0]:
            raise Exception("Invalid results. Ground truth and query vectors should be of the same length.")
