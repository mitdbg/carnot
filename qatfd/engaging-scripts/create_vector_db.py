...
dict to `(doc_id, document_text, extra_metadata)`. To add a benchmark, write an
adapter and register it. The generic machinery below — metadata loading,
rank-sharding, the resume manifest, and batched upserts — is corpus-agnostic.

Usage (was `python -m skunk.search_agent.prep.create_vector_db` before 2026-07-07,
then imported `run_build` from skunk until 2026-07-08 when it was inlined here):
