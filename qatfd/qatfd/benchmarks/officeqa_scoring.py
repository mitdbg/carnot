"""OfficeQA answer scoring — the official Grounded Reasoning Cup reward, vendored.

The competition grades submissions with a deterministic reward function whose
canonical copy ships with the competition harness
(``skunk/harness_ui/cup_kit/reward.py``, byte-identical to the reward the cup
runs server-side). qatfd owns this vendored copy because qatfd is where systems
are evaluated against benchmarks; the skunk library deliberately ships no
scorer.

Everything between the BEGIN/END markers below is copied **unchanged** from
``cup_kit/reward.py`` at cup-kit version 0.1.6 (the reward the kit's
``cup_kit/scorer.py`` wrapper reports as ``officeqa.cup.scoring.v5``). If the
upstream reward changes, re-copy the function bodies and bump
``SCORER_VERSION``. The single qatfd-specific addition is :func:`score_correct`
at the bottom.

History: qatfd previously imported ``skunk.eval.scoring``, which vendored the
older ``officeqa.cup.scoring.v4`` reward. v5 adds a bounded thousands-separator
regex (a decimal before a comma is a list delimiter, not a thousands group) and
a gold-list segmentation path (a prediction like ``[8,152,260]`` can match a
two-item gold ``[8, 152260]``). Report rows record the scorer version, so v4
and v5 runs are distinguishable.

Usage
-----
    from qatfd.benchmarks.officeqa_scoring import score_correct
    score_correct(gold_answer, predicted)  # -> 1 if correct at 0.0% rel-err else 0
"""

from __future__ import annotations

# ---------------------------------------------------------------------------
# BEGIN vendored upstream reward.py (do not edit — re-copy on upstream change)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# END vendored upstream reward.py
# ---------------------------------------------------------------------------

# The Scorer.VERSION the cup kit's scorer.py wrapper reports for this reward.
