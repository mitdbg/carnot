...
The semantic-filter tool is skunk's first-class `semantic_filter`; the agent builds it from
`SearchAgentConfig` (`semantic_filter_model` / `semantic_filter_provider_order` / caps).
"""
...
class AblationSearchAgentSystem(SearchAgentSystem):
    ...
    def _include_semantic_filter(self) -> bool:
        return self.config.tool_semantic_filter
