...
- Retrieval is **page-level**: `PageRef.page` is the **1-based PDF page index** — the only page-number convention used in the codebase. `RetrieveExecutor` in `src/skunk/retrieve.py` is currently a placeholder (golden-bypass only; raises `NotImplementedError` otherwise) pending integration of an external retriever; the previous page-index pipeline is preserved at `src/skunk/page_index/retrieve_prototype.py` (`PageIndexRetrievePrototype`).
