...
- Retrieval is **page-level**: `PageRef.page` is the **1-based PDF page index** — the only page-number convention used in the codebase. `RetrieveDispatcher` in `src/skunk/retrieve.py` selects one of three interchangeable `RetrieveStrategy` executors per call: golden bypass first (`GoldenRetrieveExecutor`, when `ctx.config.golden_pages` is set), then routes per `config.retriever` to `"search_agent"` (default — `SearchAgentRetrieveExecutor`, wrapping the iterative ChromaDB + LLM-loop retriever vendored under `src/skunk/search_agent/`; returns page keys that are mapped to `PageRef` via `models.page_key_to_pageref`) or `"page_index"` (`PageIndexRetrieveExecutor`, wrapping the page-index retriever at `src/skunk/page_index/query.py:PageIndexRetriever` — ToC chapter pick → year filter → two-stage parallel semantic filter). The search agent needs `cache/chromadb/` + `cache/clean_page_map.json` built offline (see `src/skunk/search_agent/prep/`); first non-golden call fails fast with a clear error if either is missing.
...
- `src/skunk/orchestrator.py` — `Orchestrator(ctx).execute()` — Plan executor; instantiates one operator-level class per op (`RetrieveDispatcher` / `ExtractExecutor` / `ComputeExecutor`, plus `LookupExternalPromptedCall` which doubles as both operator and call-site since `lookup_external` is a single LLM call) and dispatches through their `.run()` methods
...
- **Logging — log each fact at the layer that owns it, and only there** (full rules in `ARCHITECTURE.md` → "Logging & observability"). Two systems: the **trace** (`trace.py`, caller-owned operator boundaries — op/timing/result/error, populated by the orchestrator's `execute_with_tracing`) and the **event stream** (`ctx.emit(source, message, **fields)`, callee-owned internal detail). Rules: caller owns boundaries → operators do NOT emit their own `"starting"`/`"done"`; LLM I/O is logged once at `LLMClient` (parse retries at `PromptedCall`); `message` is a stable snake_case key with all variables in `**fields` (no interpolation); request-scoped events use `ctx.emit`, process-scoped code with no ctx (build/offline prep, `LLMClient` retries) uses `logging.getLogger(__name__)`. `_step` is reserved for the orchestrator's trace/event join key.
