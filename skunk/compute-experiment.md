...
1. **Add a value-conflict check to extract** — UID0029 (23.5% off), UID0027 (3.6%), UID0245 (underflow), UID0114 (8.6%) all stem from extract returning multiple entries with the same / near-identical description but different values. Extract has no dedup at all, so description-match + value-mismatch duplicates flow through. Add a verifier that flags or merges these. **~4 UIDs**.
