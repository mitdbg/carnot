...
The correction loop is exercised with a fake agent whose `call()` replays a script, so no LLM,
chroma, or tools are constructed. Runs under pytest, or standalone:
...
class _FakeSearchAgent(SearchAgent):
    """A SearchAgent whose `call()` replays a script of final-answer payloads, bypassing the
    heavy real __init__ (no chroma / tools). Records each call's kwargs so the test can assert
    the correction turns resume the conversation on the tight budget."""
