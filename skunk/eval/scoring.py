...
Delegates to the competition kit's reward (``harness_ui/cup_kit/reward.py``,
byte-identical to the reward the cup runs server-side) — the single scoring
implementation in this repo. The skunk *library* deliberately ships no scorer:
the old ``skunk.eval.scoring`` module vendored a stale v4 copy of the reward
and was removed; qatfd owns its own vendored copy
(``qatfd/benchmarks/officeqa_scoring.py``).
"""
...
# cup_kit lives under harness_ui/, which is not a package; make it importable
# the same way the harness scripts do (harness_ui/ itself on sys.path).
_HARNESS_UI = str(Path(__file__).resolve().parents[1] / "harness_ui")
if _HARNESS_UI not in sys.path:
    sys.path.insert(0, _HARNESS_UI)

from cup_kit.reward import score_answer  # noqa: E402
