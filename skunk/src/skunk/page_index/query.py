...
most once — judging each page against the branch RETRIEVAL TARGETS (a single relevant/not
verdict, true if the page fits ANY target). Judging the clean targets rather than the raw
computation-heavy question is what lets the coarse model keep on-topic pages. A
question-scoped decision cache keeps a page from being judged twice across sibling branches
(per-branch `run` calls share it via the ctx). Survivors route back to a branch iff that
branch's cheap pass kept the page AND the filter marked it relevant."""
...
"""
    ...
    @staticmethod
    def _decision_cache(ctx: ExecutionContext) -> dict[PageRef, bool]:
        """The question-scoped page→relevance cache, lazily created on the ctx. One ctx per
        question (single-threaded — see `ExecutionContext`), so this judges each unique page
        at most once across a question's sibling branches: the union inside one `retrieve_all`
        dedups within a call, and this cache carries decisions across the separate per-branch
        `run` calls too. Freed with the ctx — no cross-question leak."""
    ...
    async def _semantic_filter(
        ...
        period) — true if the page fits ANY target. Pages are exploded into their CONTENT BLOCKS
        and judged as one FLAT list (each block its own row), not as nested page objects: the flat
        shape keeps the coarse model's per-item judgments stable at larger batches (nested
        page→blocks objects degrade fast as the batch grows). One boolean per block comes back; a
        page is kept iff ANY of its blocks is relevant. Judging clean targets rather than the raw
        (computation-heavy) question is what lets the coarse model keep on-topic pages.
        ...
        async def _one_batch(
            ...
            try:
                ...
                verdicts = [True] * n
            # Page kept iff ANY of its blocks judged relevant.
            out: dict[PageRef, bool] = {pk: False for pk, _ in batch}
        ...
        for d in per_batch:
            cache.update(d)
        n_kept = sum(1 for pk in pages if cache[pk])
        n_blocks = sum(len(rows) for _, rows in groups)
        ctx.emit(
            f"semantic_filter model={model} kept={n_kept}/{len(pages)} "
            f"judged={len(todo)} cached={len(pages) - len(todo)} blocks={n_blocks}"
        )
    ...
    async def retrieve_all(
        ...
        (ToC pick → year filter); the deduped union is then filtered once, judging every page
        against the branch RETRIEVAL TARGETS (one verdict per page, true if it fits ANY target).
        A page routes to branch `i` iff `i`'s cheap filters kept it AND the filter marked the
        page relevant. Result is aligned to `branches`; each branch's anchors are expanded
        into member refs.
        ...
        # ToC+date recall-ceiling eval mode, which keeps every cheap candidate).
        if ctx.config.retrieve_skip_semfilter:
            verdict = {ref: True for ref in unique}
            ctx.emit(f"semantic_filter SKIPPED kept={len(unique)}/{len(unique)}")
        else:
            verdict = await self._semantic_filter(unique, branches, ctx)

        # Route + expand anchors per branch, preserving each branch's candidate order.
        out: list[list[PageRef]] = []
        for i, refs in enumerate(cand):
            kept = [ref for ref in refs if verdict[ref]]
            # Expand each kept anchor into its member refs (the contiguous anchor + folded
            # continuation run). The store serves the anchor's merged text once; the vision
            # tier renders every page of the table.
            expanded = [
                r
                for ref in kept
                for r in (
                    self._catalog[ref].member_refs() if ref in self._catalog else [ref]
                )
            ]
            b = branches[i]
            ctx.emit(
                f"page_index_retrieve key={b.key!r} period={b.period!r} as_of={b.as_of!r} "
                f"catalog_size={self._catalog_size} anchor_count={len(kept)} candidate_count={len(expanded)} "
            )
