...
class _LLMBackend:
    ...
    def embed_query(self, text: str, *, model: str, usage_key: str = "default", http_headers: dict[str, str] | None = None, timeout_s: float | None = None) -> list[float]:
        ...
        def attempt() -> list[float]:
            t0 = time.monotonic()
            vector, in_tok = self._embed_once(model, text, http_headers=http_headers, timeout_s=timeout_s)
...
class _OpenRouterBackend(_OpenAIChatBackend):
    ...
    def _embed_once(self, model: str, text: str, http_headers: dict[str, str] | None = None, timeout_s: float | None = None) -> tuple[list[float], int]:
        ...
        resp = self._get_client().embeddings.generate(
            input=text,
            model=model,
...
class _VLLMBackend(_OpenAIChatBackend):
    ...
    def _embed_once(self, model: str, text: str, http_headers: dict[str, str] | None = None, timeout_s: float | None = None) -> tuple[list[float], int]:
        ...
        resp = (
            ...
            else client.embeddings.create(model=model, input=text, timeout=timeout_s)
        )
        data = getattr(resp, "data", None) or []
        vector = list(data[0].embedding) if data and data[0].embedding else []
        if not vector:
...
class LLMClient:
    ...
    def embed_query(
        ...
        *,
        ...
        return self._backend(provider).embed_query(text, model=model, usage_key=usage_key, http_headers=http_headers, timeout_s=timeout_s)
