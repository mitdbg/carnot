"""Official OfficeQA Cup answer scoring, vendored into the skunk library.

The competition grades submissions with a deterministic reward function. The
canonical implementation ships in the cup kit (``cup_kit/reward.py``), whose own
docstring declares it *byte-identical to the reward the cup runs server-side*.
That kit is a local-only artifact and is not committed to this repo, so the
scoring logic below is vendored here verbatim (functions copied unchanged from
``officeqa-cup-kit-v0.1.5/cup_kit/reward.py``) — the skunk library owns its own
copy and depends on nothing external to grade answers.

Scorer version: ``officeqa.cup.scoring.v4`` (the ``Scorer.VERSION`` the kit's
``cup_kit/scorer.py`` wrapper reports for this reward).

If the upstream reward changes, re-copy the function bodies below and bump
``SCORER_VERSION``. The single skunk-specific addition is :func:`score_correct`
at the bottom; everything above it is upstream and should not be edited.

Usage
-----
    from skunk.eval.scoring import score_correct
...
# The Scorer.VERSION reported by the cup kit's scorer.py wrapper for this reward.
SCORER_VERSION: str = "officeqa.cup.scoring.v4"
