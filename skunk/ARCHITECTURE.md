...
- **`page_index`** (default) — the page-index retriever at `src/skunk/page_index/query.py` (`PageIndexRetriever`), querying the offline-built catalog + concept tree, followed by the intrinsic `block_select` narrowing stage (`RetrieveOp._select`). See its query path below.
...
- **Dual-channel step protocol.** Each step the model emits fenced block(s), and the fence language is the channel: a ` ```python ` block is a tool call (exec'd in the `LocalPythonExecutor` with the agent's tools bound; its output becomes the next observation), and a ` ```json ` block is the final answer (parsed directly as data — never exec'd — and gated by `validate_final_answer`). The final answer is data, so it leaves as data: there is no `final_answer` tool, `FinalAnswerException`, or `is_final_answer` flag routing it back through the sandbox. A stray `final_answer(...)` call in a python block just raises `NameError` and is fed back as a misfire. `call()` holds all per-question state in locals, so one agent instance is reentrant across concurrent questions.
- **Parallel tool calls (`max_parallel_tool_calls`).** A step may emit **1..N** ` ```python ` blocks (N = `MultiTurnAgent.max_parallel_tool_calls`, config-derived per agent: `search_agent_max_parallel_tool_calls` / `lookup_max_parallel_tool_calls` default 3; the stateful `task_solver` stays at 1). When N>1 the blocks run **in parallel** and return as one batched observation (per-call `--- Tool call i/n ---` headers), letting the search agent fire several independent `search_corpus` / `grep_corpus` queries at once. Rules enforced by `_prepare_codes`: emit more than N → run the first N, warn; `prune(...)` (mutates the shared pruned-id sets the searches read) or the ` ```json ` final answer must each be a step **by themselves** → a mixed prune / final-answer block is dropped with a `[notice]`. The streaming `should_stop` (`_make_should_stop`) stops once N blocks complete or a json block appears; N=1 is exactly the original "stop at first block" behaviour.
- **Parallel execution must not block the loop.** Within a question, retrieve branches run as concurrent `asyncio` tasks on **one event loop in one worker thread** (one loop per question — see `eval_e2e._run_uid`). A step's tool blocks are therefore executed via `await asyncio.gather(*(asyncio.to_thread(_run_block, executor, code) ...))` in `MultiTurnAgent._execute_codes`: the `await` yields the loop so sibling branches keep progressing while the blocking tool I/O (embeddings + ChromaDB) runs on threads — a blocking `ThreadPoolExecutor(...).result()` here would stall the whole question. A single-block step reuses the **persistent** executor (cross-step interpreter state survives, which `task_solver` relies on); a parallel batch gives each block its **own fresh** executor so concurrent calls can't race on interpreter state (`_print_outputs` / `state`). Tools never touch `ctx`, so off-loop execution stays thread-safe; the process-wide `embed` rate limiter bounds real API concurrency.
