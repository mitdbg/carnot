...
**Two retrieval methods.** `RetrieveOp.run_all` in `src/skunk/retrieve.py` is the **single retrieval seam**: it is the only layer that inspects `ctx.config.golden_pages` / `config.retriever`, and it normalizes every backend to one per-branch result — `BranchRetrieval(blocks)` (`src/skunk/common.py`). Every backend's `blocks` are already final (the selection has happened inside the backend), so downstream they go straight to extract. A branch that fails (e.g. the sem filter keeps nothing) surfaces as a per-branch `StepFailed` in the same list, so one branch failing doesn't sink its siblings. Dispatch:

- **`page_index`** (default) — the page-index retriever at `src/skunk/page_index/query.py` (`PageIndexRetriever`), querying the offline-built catalog + concept tree (see its query path below). Its sem-filter survivors are then narrowed by ONE **`SelectAgent`** (`src/skunk/select_agent/`): given the full question and the UNION of every (non-pinned) branch's flagged survivors, the agent selects the pages that answer the whole question and writes a per-page retrieval target. The candidate pool is built once from the union via `PageIndexRetriever.pool_for_blocks` (the canonical blocks→pool builder); the agent runs over a catalog + `PageStore` (no ChromaDB). Its selected pages are broadcast as whole-page blocks to every non-pinned branch (each branch's extract then pulls its own series from them). Pinned branches (`@pin`) stay deterministic positional fetches and skip the agent. Each unique selected block is then read ONCE (text tier, vision fallback), serving every branch that selected it.
- **`search_agent`** — the iterative ChromaDB + LLM-loop retriever vendored under `src/skunk/search_agent/`. Its page-key output is wrapped as whole-page blocks.
