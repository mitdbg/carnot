...
- **`search_agent`** — the iterative ChromaDB + LLM-loop retriever vendored under `src/skunk/search_agent/`. Its page-key output is wrapped as whole-page blocks with `pre_selected=True`.
  - *Optional `page_index_search` tool* (env `SKUNK_SEARCH_AGENT_PAGEINDEX=1`, default off): adds a tool that lets the search agent query the `page_index` retriever (concept-tree → year filter → semantic filter) from inside its loop, blending concept-aware retrieval with vector search. It wraps `PageIndexRetriever.retrieve_all` over one synthetic branch and returns the surviving blocks as catalog summaries keyed by `page_key` (so they round-trip through `read_document` / the final answer via `pageref_to_page_key`). The shared `PageIndexRetriever` is passed in from `RetrieveOp._run_search_agent` (built once), and the tool drives the async retriever via `asyncio.run` — safe because tool calls run in a worker thread and the retriever's rate limiters are cross-loop safe. This is a new *tool*, not a new operator. Requires the `page_index` artifacts to be present alongside the search-agent artifacts.
