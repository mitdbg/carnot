...
- **`search_agent`** — the iterative ChromaDB + LLM-loop retriever vendored under `src/skunk/search_agent/`. Its page-key output is wrapped as whole-page blocks with `pre_selected=True`.
  - *Optional `page_index_search` tool* (env `SKUNK_SEARCH_AGENT_PAGEINDEX=1`, default off): adds a tool that lets the search agent query the `page_index` retriever (concept-tree → year filter → semantic filter) from inside its loop, blending concept-aware retrieval with vector search. It wraps `PageIndexRetriever.retrieve_all` over one synthetic branch and returns the surviving blocks as catalog summaries keyed by `page_key` (so they round-trip through `read_document` / the final answer via `pageref_to_page_key`). The shared `PageIndexRetriever` is passed in from `RetrieveOp._run_search_agent` (built once). Tool calls run in a worker thread (`MultiTurnAgent._execute_code` → `asyncio.to_thread`), but the tool must drive the async retriever ON the agent's main event loop — the LLM client's async objects (TPM limiter, genai/aiohttp session) are bound to it, so a fresh `asyncio.run` loop would raise "Future attached to a different loop". The tool therefore schedules the coroutine back onto the captured main loop via `run_coroutine_threadsafe` and blocks the worker thread on the result (the main loop keeps spinning while awaiting the `to_thread` call). This is a new *tool*, not a new operator. Requires the `page_index` artifacts to be present alongside the search-agent artifacts.
...
- **`compute()`** — chain terminator that subsumes formatting. Reads `ctx.question` plus the upstream extracted/looked-up values; runs a single loop (`ComputeOp.run`) of codegen → in-process exec under one shared budget (`compute_max_attempts`, default 3). Each iteration's codegen call emits one of two `python` forms: **(a)** assign `result` (success); or **(b)** *missing data* — assign `keep` (a `{name: value}` dict of everything to carry into the next round: the inputs the model re-states plus any partial computation it derived, e.g. the qualifying month of a multi-hop question) **and** `missing` (the identifiers + a one-line reason). `keep` is drop-by-default — anything not in it is dropped for good (there is no give-up form that silently keeps everything). Parse, exec, and malformed-block failures feed the next iteration's `prev_code` + `prev_failure`. `run()` returns a union: `Final(answer)` on a clean exec that set `result`, or `NeedsMore(keep, missing)` — it never raises `MissingData`. Fails with `StepFailed("compute", …)` when no iteration ever resolves. Across `compute_best_of_n` trials, `_vote` returns the most-frequent outcome with all `NeedsMore`s pooled into a single "missing-data" candidate (a tie never breaks in favor of missing-data); when missing-data wins, the trials' `keep`s are unioned and their reasons rendered per agent ("agent 1: …, agent 2: …").
  ...
  **Optional multi-turn variant** (`SKUNK_COMPUTE_AGENT=1`): `make_compute_op` swaps `ComputeOp` for `ComputeAgentOp` (`src/skunk/compute_agent.py`), a `MultiTurnAgent` that writes/executes Python step-by-step over the same pre-loaded `input_values` AND can call the corpus tools (`search_corpus` / `grep_corpus` / `read_document` / `view_figure`) — but only to *contextualize* its inputs (resolve a unit/footnote/scope/figure), never to retrieve primary data from scratch; absent data still emits `NeedsMore`. Its `json` final answer is `{"result": ...}` (→ `Final`) or `{"missing": [...], "description": ..., "keep_inputs": [...], "keep_computed": {...}}` (→ `NeedsMore`, mapped back to `AnnotatedValue`s — inputs by index keep their provenance, computed values are provenance-free). Same `run(...)` signature, the same best-of-N `vote_compute_outcomes`, and the same shared `build_compute_user_message`/keep-construction as the single-shot path. Requires the search-agent corpus artifacts (chromadb + clean_page_map), like `SKUNK_RETRIEVER=search_agent`.
...
- **Agentic loops confined to retrieve and lookup_external.** Two operators run iterative tool-using LLM loops (both `MultiTurnAgent`s): retrieve when `config.retriever == "search_agent"` (`SearchAgent` in `src/skunk/search_agent/`), and `lookup_external` always (`LookupAgent`). `extract` and `compute` do not loop agentically by default — they execute once per call (compute's internal codegen→exec retries are a fixed bounded budget, not a tool loop), and failure is recorded in the trace. (Exception: `SKUNK_COMPUTE_AGENT=1` makes compute a third `MultiTurnAgent` loop — see `compute()` above.) Crucially, *missing-data recovery* is a bounded replan loop in the orchestrator — the planner returns a `PlanDiff` (add / drop / resume) and `_apply_diff` carries it out — not a per-operator retry loop: an operator never re-plans or re-dispatches itself on failure.
