...
- **`compute()`** — chain terminator that subsumes formatting. Reads `ctx.question` plus the upstream extracted/looked-up values; runs a single loop (`ComputeOp.run`) of codegen → in-process exec under one shared budget (`compute_max_attempts`, default 3). Each iteration's codegen call may return Python (success), raise `MissingData` (the structured insufficient-data signal), or raise `_ParseFailure` (unparseable reply); parse and exec failures feed the next iteration's `prev_code` + `prev_failure`. Returns the result of the first clean exec as a plain `str`. Fails with `StepFailed("compute", …)` when no iteration ever execs cleanly; propagates `MissingData` — the orchestrator catches that and runs up to `recovery_max_rounds` re-planning rounds before giving up.
  ...
  Each autonomous recovery round runs `Planner.replan`, which returns a **`PlanDiff`** — a delta against the prior plan, not a fresh plan — with three operations: `add` (new branches to run), `drop` (indices into the prior branch list to remove), and `resume` (`{index, feedback}` ops continuing an existing retrieve branch's search). The orchestrator applies it **positionally** (`Orchestrator._apply_diff`), keeping `outcomes[i]` aligned with `plan.branches[i]`: a dropped index removes the branch *and* the data it gathered (so compute never sees retracted data); added branches are appended; unchanged branches keep their already-gathered outcomes and are never re-run. For each `resume` op, the orchestrator continues that kept branch's **retained agent** (the `search_agent` backend registers every branch's agent in `Orchestrator._retrieve_agents`, keyed by stable branch id) with the planner's `feedback` on a deliberately SMALLER step budget (`MultiTurnAgent.resume` → `_run_loop`; the initial search gets `agent_max_steps`=20, each resume only `agent_resume_max_steps`=10), then re-runs extract over the **union** of the branch's prior + newly-found blocks (a previously-successful branch never loses data it already found) and replaces that branch's outcome in place (`Orchestrator._resume_one`). A `resume` op on a dropped index or a branch with no retained agent (e.g. a lookup branch) is skipped with a note. So the planner chooses, per branch: resume it with targeted feedback (it found *part* of the data — keep looking), drop it and add a `lookup_external` (the value isn't in the corpus), add a new branch, or leave it. Recovery rounds share the `recovery_max_rounds` budget; the human-in-the-loop path also drives recovery through the same `replan` → `_apply_diff`.
...
**Both branch kinds are first-class on the initial plan.** The planner chooses `retrieve` vs `lookup_external` per value (`_INITIAL_PASS_RULE`, the corpus-biased-but-corpus-agnostic source-selection rule): prefer `retrieve` whenever the data could reasonably be expected to live in the corpus, and emit a `lookup_external` up front for a value unlikely to be in the corpus or when the question names an external source — rather than defaulting everything to `retrieve` and deferring external lookups. `_parse_plan` permits both kinds initially (it enforces only `as_of` parity and the retrieve-key-verbatim rule). At replan, `Planner.replan` can still add a `lookup_external` (e.g. when a retrieve branch's search showed the value isn't in the corpus) or resume/redirect a retrieve branch via `PlanDiff`. (The `src`-verbatim rule — a lookup may pin a publisher only when the question names it — lives in `_parse_plan_diff`; it's a no-op for a null `src`.)
...
- `LookupBranch(target, src=None)` — single `lookup_external` call. Allowed on the initial plan and addable at replan (see above). `src` is set only when the question names a publisher (verbatim), enforced in `_parse_plan_diff`.
