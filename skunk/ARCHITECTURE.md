...
- **`compute()`** — chain terminator that subsumes formatting. Reads `ctx.question` plus the upstream extracted/looked-up values; runs a single loop (`ComputeOp.run`) of codegen → in-process exec under one shared budget (`compute_max_attempts`, default 3). Each iteration's codegen call emits one of three forms: **(a)** Python assigning `result` (success), **(b)** a bare missing-data JSON object (give-up: nothing computable), or **(c)** Python that assigns `missing` instead of `result` — *partial progress*, where executed code computes intermediates from the gathered values (e.g. the qualifying month of a multi-hop question), commits them via a `committed` dict, and MUST name the inputs that survive into the next round via a required `keep` index list (everything unlisted is dropped). Parse, exec, and malformed-form-(c) failures feed the next iteration's `prev_code` + `prev_failure`. `run()` returns a union: `Final(answer)` on the first clean exec that set `result`, or `NeedsMore(keep, committed, missing)` for forms (b)/(c) — it never raises `MissingData`. Fails with `StepFailed("compute", …)` when no iteration ever resolves.

  Recovery state is a **value pool**, not branch outcomes: the orchestrator seeds it with the initial sweep's `AnnotatedValue`s, and on each `NeedsMore` keeps ONLY the `keep`-indexed entries (compute's explicit whitelist — drop is the default) and appends the `committed` computed intermediates (provenance-free, description-marked `computed intermediate (round N)`); discards are gone for good. Each recovery round (≤ `recovery_max_rounds`), `Planner.replan` composes a **fresh `Plan`** containing only the branches to run now — no diff, no index alignment; branch identity dies at the end of its sweep. The replan message carries the kept pool's schema, the **previous plans** (every branch tried with its fate + failure diagnostics), and compute's missing description, and the replanner is instructed to diagnose why the gathering so far did not satisfy compute before emitting branches — never re-emitting an approach that already failed the same way. A replan sweep that yields zero new values **short-circuits**: compute is skipped (it would be a no-op on an unchanged pool) and the prior `NeedsMore` drives the next replan, still consuming a round. When the budget is exhausted, `execute()` raises `MissingData` — its only remaining role, the terminal signal the eval harness records.
...
**Lookups default to retrieve.** The planner is instructed (advisorily — there is no parse-hook enforcement) to emit `lookup_external` on the initial plan only when the statistic is chronologically outside what the corpus can cover or the question names a clear external source; at replan, `lookup_external` is unrestricted. There are no verbatim checks on retrieve `key`s or lookup `src`s — both are free natural language.
...
The eval harness keeps a **retrieval cache** for iteration, not a runtime cache: every real `eval/eval_e2e.py` run auto-writes its retrieve output (`Orchestrator.retrieved_pages`, the deduped page union per UID) to `<run-dir>/retrieval_cache.json`, and `--retrieval-cache <path>` replays one by injecting those pages golden-style (bypassing the retrieve operator) so extract/compute can be tuned without re-paying retrieval. Cached state is the retrieve phase's output (the sem-filter survivor pool plus the deduped block union), so a replay runs selection/extract over exactly what the original run saw.
