...
- `RetrieveBranch(key, period, as_of=None, visual_only=False)` — `retrieve` then `extract` are dispatched for the branch by the orchestrator (extract reads `visual_only` from the branch; `as_of` is the reporting/vintage **publication** month — used by retrieve to pin the issue (era pruning + year filter, with `block_select` as the exact-issue tie-breaker) and stamped onto each extracted value as provenance for compute; it does **not** otherwise steer extraction).
